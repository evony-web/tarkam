  ...
  return (
    ...
    >
      <div className="relative h-3.5 w-3.5 overflow-hidden">
        <Sun className={`absolute inset-0 h-3.5 w-3.5 transition-all duration-300 ${isDark ? 'rotate-90 scale-0 opacity-0' : 'rotate-0 scale-100 opacity-100'}`} />
        <Moon className={`absolute inset-0 h-3.5 w-3.5 transition-all duration-300 ${isDark ? 'rotate-0 scale-100 opacity-100' : '-rotate-90 scale-0 opacity-0'}`} />
      </div>
    </button>
  );
}
      ...
      <button
        ...
        className={`btn-press flex items-center gap-1.5 px-2 sm:px-2.5 py-1 rounded-full transition-all duration-200 cursor-pointer border active:scale-95 ${
          scrolled
            ...
            : 'border-white/15 bg-white/5 hover:bg-white/10 dark:border-white/15 dark:bg-white/5 dark:hover:bg-white/10'
        }`}
      >
